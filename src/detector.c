...
float quantize_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && (i + t) < m; ++t) {
            const int image_index = (i + t) % 1000;
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            const int image_index = (i + t - nthreads) % 1000;
...
void run_detector(int argc, char **argv)
{
    ...
    else if (0 == strcmp(argv[2], "quantize")) quantize_detector_map(datacfg, cfg, weights, thresh, iou_thresh, map_points, letter_box, NULL);
